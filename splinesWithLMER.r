...
###
 ...
 tmp<-names(table(Y$VAR))[table(Y$VAR)>50]
 W<-W[Y$VAR%in%tmp,]
 Y<-Y[Y$VAR%in%tmp,]
 ...
 msg0<-c( c('ll_Int','ll_LiF' ,'ll_SpF' ,'ll_LiR' ,'ll_SpR'),
 		 c('var_Int','var_LiF','var_SpF','var_LiR','var_SpR'),
 		 c('BIC_Int','BIC_LiF','BIC_SpF','BIC_LiR','BIC_SpR')
 	)
 ...
 for(i in covNum){
  ...
  msg<-c( logLik(fmInt),logLik(fmLiF),logLik(fmSpF),logLik(fmLiR),logLik(fmSpR),
  	      summary(fmInt)$sigma^2,summary(fmLiF)$sigma^2,summary(fmSpF)$sigma^2,summary(fmLiR)$sigma^2,summary(fmSpR)$sigma^2,
  	      BIC(fmInt),BIC(fmLiF),BIC(fmSpF),BIC(fmLiR),BIC(fmSpR)
  	    )
  names(msg)=msg0
  print(round(msg,2))      
